...
//		var msg = "{suid:"+psgid+"}";
//		$.ajax({
//	    url:'http://221.204.237.67:8099/Android_Server/subscribe!android_subscribe',// 跳转到 action
//			data:{
//				msg:msg
//			},
//	    type:'post',  
...
//			var msg ="{suid:'592061,616996,589043,616472,616409',startdata:'"+startDate+"',enddata:'"+endDate+"'}";
			...
			$.ajax({
				type: 'post',
				url: "http://221.204.237.67:8099/Android_Server/Distance",
				cache: false,
				data: {
					msg:msg
				},
...
//			var msg ="{suid:'592061,616996,589043,616472,616409',startdata:'"+startDate+"',enddata:'"+endDate+"',opid:"+this.opid+"}";
			...
			$.ajax({
				type: 'post',
				url: "http://221.204.237.67:8099/Android_Server/Alarm",
				cache: false,
				data: {
					msg:msg
				},
...
//					alert("数据查询失败，请稍候再试");
	...
	Model.prototype.sendPositionToMap = function(positionData,type){
			...
			this.comp("mapContainer").refresh("$UI/CHDPer/mapActivity.w",{data:sendData});
...
//			var vname ="吉AH1931";
			...
			$.ajax({
				type: 'post',
				url: "http://221.204.237.67:8099/Android_Server/suidtrack",
				cache: false,
				data: {
					msg:msg
				},
...
//			this.comp("mapReplayContainer").load("$UI/CHDPer/mapReplayActivity.w",{data:sendData});
			this.comp("mapReplayDialog").open({"src":"$UI/CHDPer/mapReplayActivity.w","data":sendData});
	};	//轨迹回放点轨迹标志
	...
	Model.prototype.devicePicRowClick = function(event){//照片查询
			...
			$.ajax({
				type: 'post',
				url: "http://221.204.237.67:8099/Android_Server/VID_Image",
				cache: false,
				data: {
					msg:msg
				},
				...
				success : function(msg) {
					if(msg.success){
						...
						if(msg.size===0){
							...
							self.comp("picViewDialog").open({"src":"$UI/CHDPer/pictureView.w","data":data});
						}
					}else{
						justep.Util.hint("该车并未上传车辆照片");
					}
	...
	Model.prototype.unitDetailRowClick = function(event){
			...
			$.ajax({
				type: 'post',
				url: "http://221.204.237.67:8099/Android_Server/VehicleInfo",
				cache: false,
				data: {
					msg:msg
				},
